...
var palette = {
	...
	diverging: function diverging(index) {
		...
		switch (index) {
			...
			case 4:
				// Custom
				return ["#777", "#7792AE", "#D95B43", "#aaa", "#ffe600", "#ffcfb4", "#ff9e7a", "#ff6842", "#ff0000"];
		}
	},
...
function chartPolarAreaChart () {
  ...
  */
	function my(selection) {
		...
		var layers = [ "polarArea"];  //["circularAxis", "polarArea", "circularSectorLabels", "verticalAxis axis"];
		chart.classed(classed, true).attr("transform", "translate(" + width / 2 + "," + height / 2 + ")").attr("width", chartW).attr("height", chartH).selectAll("g").data(layers).enter().append("g").attr("class", function (d) {
			return d;
		});
		...
		selection.each(function (data) {
			...
			chart.select(".circularSectorLabels").attr("color", "white").call(circularSectorLabels);
		});
	}
